...
import (
	...
	clusterv1alpha1 "go.bytebuilders.dev/crd-learner-template/apis/cluster/v1alpha1"
	clientset "go.bytebuilders.dev/crd-learner-template/client/clientset/versioned"
	samplescheme "go.bytebuilders.dev/crd-learner-template/client/clientset/versioned/scheme"
	clusterInformer "go.bytebuilders.dev/crd-learner-template/client/informers/externalversions/cluster/v1alpha1"
	clusterLister "go.bytebuilders.dev/crd-learner-template/client/listers/cluster/v1alpha1"
...
func (c *controller) syncHandler(key string) error {
	...
	if cluster.DeletionTimestamp != nil {
		...
		bookDeploymentName := cluster.Spec.BookDeploymentName
		...
		if *bookDeployment.Spec.Replicas != *cluster.Spec.BookReplicas && cluster.Spec.BookReplicas != nil {
			klog.V(4).Infof("cluster %s replicas: %d, deployment replicas: %d", name, *cluster.Spec.BookReplicas, *bookDeployment.Spec.Replicas)
...
func (c *controller) updateClusterStatus(cluster *clusterv1alpha1.Cluster,
	...
	if cluster.DeletionTimestamp == nil {
		...
		clusterCopy.Status.BookAvailableReplicas = bookdeployment.Status.AvailableReplicas
	}
...
func newDeployment(cluster *clusterv1alpha1.Cluster) *appsv1.Deployment {
	...
	return &appsv1.Deployment{
		...
		Spec: appsv1.DeploymentSpec{
			Replicas: cluster.Spec.BookReplicas,
